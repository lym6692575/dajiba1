...
describe('test CategorySelect component', () => {
    ...
    it('render selectedCategroy with category item with hightlight', () => {
        const wrapper = mount(<CategorySelect{...props_with_category} />)
        expect(wrapper.find('.category-item').first().hasClass('active'))
    })
    it('click the item should add active class and tirgger the callback',()=>{
        const wrapper = mount(<CategorySelect{...props_with_category} />)
